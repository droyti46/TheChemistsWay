...
[1, 2, 3, 4]
```
Также в массиве массивов, которые надо объединить, могут находиться не только массивы, но и одиночные объекты. Они тоже добавятся в итоговый массив
```gdscript
var arr1: Array = [1, 2, 3]
var arr2: Array = [4, 5, 6]
var elem: int = 7

print(merge_arrays([arr1, arr2, elem]))
```
Вывод:
```
[1, 2, 3, 4, 5, 6, 7]
...
Проверяет, являются ли координаты допустимыми. Важно вызывать перед каждым перемещением кого и чего бы то ни было в коде
```gdscript
